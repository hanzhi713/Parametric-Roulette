...
function calculateLocations(t1, t2, xExp, yExp, step, radius, scale) {
    let callAgain = false;
    const prevxExp = xExp, prevyExp = yExp;
    let cStep = step;
    let num = 1;
    while (cStep < 1) {
        cStep *= 10;
            ...
            if (Math.abs(dyE) < cuspThreshold) {
                ...
                if (currentCusp !== undefined && revolve.checked) {
                    ...
                    if (Math.abs(t - currentCusp) < maxError)
                        cNormal = lastNormal;
                    if (Math.sign(cNormal) * Math.sign(nextNormal) === -1 &&
                        t + step <= t2 &&
                        Math.abs(t - currentCusp) <= step) {
                        c_newCuspPoints.splice(c_newCuspPoints.length - 1, 1);
                        const cuspX = xFunc(currentCusp), cuspY = yFunc(currentCusp);
